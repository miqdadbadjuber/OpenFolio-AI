...
**Goal:** Implement the new Editorial/Minimalist Dark Mode redesign for the Landing Page.

**Architecture:** We are replacing the entire contents of `src/pages/LandingPage.tsx` with a static, highly typographic, pure black (`#000000`) layout. The page consists of a Header, a massive Hero section (static text), and a Feature Showcase built with an asymmetrical grid and generous whitespace.
...
- No Sparkles, Wand2, Bot, or Command icons.
- No spinning borders or dynamic text animations.
- Typographic tracking must be tight (`tracking-tighter`) for headers.

---

### Task 1: Scaffolding and Header Implementation
...
- [ ] **Step 1: Write the minimal scaffold with Header**
...
Run: `npm run build` or simply check via TypeScript compiler.
Expected: PASS
...
git commit -m "feat: scaffold landing page and header"
```
...
**Interfaces:**
- Consumes: The `main` container created in Task 1.

- [ ] **Step 1: Write the Hero implementation**

```tsx
// Inside main tag in LandingPage.tsx, replace Placeholder with:
        <section className="px-6 max-w-7xl mx-auto flex flex-col items-start min-h-[70vh] justify-center pb-20 border-b border-zinc-900">
          ...
          <div className="flex flex-col md:flex-row md:items-end gap-8 w-full">
            ...
            <Link to="/app" className="inline-flex items-center justify-center bg-white text-black px-8 py-4 font-bold text-sm uppercase tracking-widest hover:bg-zinc-200 transition-colors gap-3 shrink-0">
              Generate Portfolio
              {/* Note: Using simple right arrow, not Lucide for now, or we can add lucide import */}
...
```bash
git add src/pages/LandingPage.tsx
git commit -m "feat: implement static editorial hero section"
```

---

### Task 3: Feature Showcase Implementation

**Files:**
- Modify: `src/pages/LandingPage.tsx`

**Interfaces:**
- Consumes: `lucide-react` icons (Terminal, Layout, Globe)

- [ ] **Step 1: Add lucide imports and implement Features**
...
```bash
git add src/pages/LandingPage.tsx
git commit -m "feat: complete editorial landing page features and footer"
```
