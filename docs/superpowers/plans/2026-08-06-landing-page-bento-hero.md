# Landing Page Bento Hero — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `src/pages/LandingPage.tsx` menjadi landing dark-premium "bento hero" sesuai spec `docs/superpowers/specs/2026-08-06-landing-page-bento-hero-design.md`.

**Architecture:** Satu komponen React (`LandingPage.tsx`) berisi 7 section (Navbar, Bento Hero, Cara Kerja, Kenapa OpenFolio, Template, CTA Open Source, Footer). Semua CTA mengarah ke `/app` (guest-only). Visual hero dibangun dari markup CSS, bukan asset gambar. Data statis (daftar template, baris perbandingan, langkah) didefinisikan sebagai konstanta di dalam file.

**Tech Stack:** React 19 + Vite + Tailwind v4 (`@tailwindcss/vite`), `lucide-react` (ikon), `react-router` (`Link`), komponen `Logo` yang sudah ada. Tidak menambah dependency.

## Global Constraints

- UI-only: TIDAK mengubah logika, routing, auth, quota, atau endpoint. Hanya `src/pages/LandingPage.tsx`.
- Copy Bahasa Indonesia (sesuai spec).
- Dark premium: bg `#0a0a0f`–`#000`, teks zinc, kartu `#131318`–`#16161d`, border `white/5`–`white/10`, CTA putih (bg `#fff`, teks `#000`).
- Aksen gradient metalik (titanium `#FFFFFF` → platinum `#D4D4D8` → graphite `#71717A`) untuk highlight/ikon penting (sama dengan `Logo.tsx` `#openfolio-logo-gradient`).
- Semua tombol aksi utama → `/app`. Link GitHub → `https://github.com/MiqdadBadjuber/OpenFolio-AI`.
- Tidak ada asset gambar baru; tidak tambah dependency; tidak ada `@ts-nocheck`.
- Harus lolos `tsc --noEmit` strict (`npm run lint`) dan `npm run build`.

---

### Task 1: Rewrite LandingPage.tsx — Bento Hero

**Files:**
- Modify: `src/pages/LandingPage.tsx` (rewrite seluruh isi; file saat ini ~95 baris editorial, diganti total)
- Test: tidak ada unit test komponen (project belum punya harness component-testing); verifikasi = `npm run lint` + `npm run build` + smoke test manual (per spec "Definisi Selesai")

**Interfaces:**
- Consumes: `Logo` (`src/components/Logo.tsx`, prop `variant?: 'gradient' | 'white' | 'muted' | 'currentColor'`, `size`, `showText`), `Link` dari `react-router`, ikon dari `lucide-react` (`ArrowRight`, `Github`, `Check`, `X`, `FileText`, `Sparkles`, `Share2`, `LayoutGrid`).
- Produces: `LandingPage` (default export) — route `/` sudah terpasang di `src/App.tsx:16`; tidak ada interface baru yang dikonsumsi task lain.

- [ ] **Step 1: Setup konstanta data**

Di `src/pages/LandingPage.tsx`, di atas komponen, definisikan konstanta (copy Indonesia, nilai pasti):
...
- [ ] **Step 2: Buat wrapper halaman + ambient glow**
...
- [ ] **Step 4: Bento Hero (layar pertama, 5 kartu)**
...
<main className="relative z-10 pt-28 md:pt-32 max-w-[1200px] mx-auto px-6">
  ...
  {/* section berikutnya menggantikan komentar ini */}
</main>
```

- [ ] **Step 5: Section Cara Kerja (4 langkah)**
...
- [ ] **Step 6: Section Kenapa OpenFolio (tabel perbandingan)**
...
- [ ] **Step 7: Section Template (strip 8 kartu)**
...
- [ ] **Step 9: Tutup struktur JSX**

Pastikan semua section berada di dalam `<main>` (Step 4) dan `</div>` penutup wrapper (Step 2) menutup seluruh halaman. Hapus import yang tidak terpakai (hanya `ArrowRight`, `Github`, `Check`, `X`, `Sparkles`, `Share2`, `LayoutGrid`, `FileText`, `Link`, `Logo` yang dipakai). Tidak ada `@ts-nocheck`.

- [ ] **Step 10: Verifikasi lint & build**

Run: `npm run lint` → Expected: 0 error (tsc strict).
Run: `npm run build` → Expected: PASS (hanya warning chunk-size pre-existing yang diizinkan).
Run: `npx vitest run` → Expected: 24/24 pass (tidak ada test yang terpengaruh).

- [ ] **Step 11: Smoke test manual**

Run: `npm run dev`, buka `http://localhost:3001/`:
- Navbar fixed, blur, CTA "Deploy" → `/app`.
- Bento hero: headline "Portofolio-mu, dikodekan AI." (kata "dikodekan" gradient metalik), badge, subtext, CTA putih, 3 kartu stat (8 / /p/nama / Tanpa login).
- Card preview portfolio tampil (markup CSS).
- Section Cara Kerja 4 langkah, tabel perbandingan 6 baris, 8 template, CTA open source, footer.
- Mobile (viewport sempit): grid stack rapi, tidak overflow horizontal.
- Semua link aksi → `/app`; link GitHub → repo.
Stop dev server setelah selesai.

- [ ] **Step 12: Commit**

```bash
git add src/pages/LandingPage.tsx
git commit -m "feat: redesign landing page with bento hero layout"
```
